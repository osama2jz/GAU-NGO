import { Container, Divider, Grid, SimpleGrid, Text, useMantineTheme } from "@mantine/core";
...
export const Step2 = () => {
 ...
 const form = useForm({
  ...
  validate: {
   firstName: (value) => (value.length < 1 ? "Please enter first name" : null),
   lastName: (value) => (value?.length < 1 ? "Please enter last name" : null),
   email: (value) => (/^\S+@\S+$/.test(value) ? null : "Invalid email"),
   password: (value) => (value?.length < 8 ? "Password must at least have 8 characters" : null),
   phone: (value) => (value?.length < 8 ? "Please enter phone number" : null),
   ...
   >
    ...
    <SimpleGrid cols={3}>
     ...
     <InputField
      ...
      validateName="aob"
     />
     <InputField label="Debt" required={true} placeholder="debt" form={form} validateName="debt" />
     <InputField
      label="Housing"
      required={true}
      placeholder="housing"
